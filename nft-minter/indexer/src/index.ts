...
import { MARKETPLACE_CONTRACT_ADDRESS, ETHEREUM_NODE_URL } from "./config";
import { saveOrderFulfilled } from "./db";

// 从 artifacts/contracts/Marketplace.sol/Marketplace.json 中复制 ABI
...
async function startIndexer() {
  ...
  console.log("正在启动 Indexer，监听 OrderFulfilled 事件...");

  // 监听 OrderFulfilled 事件
  marketplaceContract.on(
    "OrderFulfilled",
    async (orderHash, seller, buyer, nftAddress, tokenId, price, event) => {
      console.log("检测到 OrderFulfilled 事件:");
      ...
      console.log("  Log Index:", event.log.logIndex);
