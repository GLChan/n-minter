...
async function startIndexer() {
    ...
    const provider = new ethers_1.ethers.JsonRpcProvider(config_1.ETHEREUM_NODE_URL);
    const marketplaceContract = new ethers_1.ethers.Contract(config_1.MARKETPLACE_CONTRACT_ADDRESS, MARKETPLACE_ABI, provider);
    console.log("正在启动 Indexer，监听 OrderFulfilled 事件...");
    // 监听 OrderFulfilled 事件
    marketplaceContract.on("OrderFulfilled", async (orderHash, seller, buyer, nftAddress, tokenId, price, event) => {
        ...
        // 保存到数据库
        await (0, db_1.saveOrderFulfilled)({
            orderHash,
            seller,
            buyer,
            nftAddress,
