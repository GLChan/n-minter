        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-wrap gap-4 mt-2">
              <Link href="/create">
                <Button variant={ButtonVariantDefault} size="lg">
                  开始创建
                </Button>
              </Link>
              <Link href="/explore">
                <Button variant={ButtonVariantSecondary} size="lg">
                  探索收藏品
                </Button>
              </Link>
            </div>
            ...
            <div className="flex items-center gap-4 mt-4">
              <div className="flex items-center -space-x-2">
                {heroData.community.avatars.map((avatar) => (
                  <div
                    key={avatar.alt}
                    className="w-8 h-8 rounded-full border-2 border-white dark:border-zinc-900 overflow-hidden relative"
                  >
                    <Image
                      src={avatar.src}
                      alt={avatar.alt}
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-400">
                ...
                <span className="font-semibold">
                  {heroData.community.count}
                </span>{" "}
                {heroData.community.description}
              </p>
            </div>
          </div>
