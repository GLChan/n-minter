...
export default function OwnedNFTsTab({ page, profile }: OwnedNFTsTabProps) {
  ...
  useEffect(() => {
    async function loadNFTs() {
      try {
        setLoading(true);
        const nftData = await getUserNFTs(page, 10, profile.id);
        setNfts(nftData);
      } catch (error) {
  ...
  return (
    <div>
      ...
      {/* 上架NFT Modal */}
      <NFTListModal isOpen={activeModalType === TransactionType.List} nft={currentItemData} onClose={closeModal} />

      {/* 取消上架NFT Modal */}
      <NFTUnlistModal isOpen={activeModalType === TransactionType.Unlist} nft={currentItemData} onClose={closeModal} />

      {/* 转移 NFT */}
      <NFTTransferModal isOpen={activeModalType === TransactionType.Transfer} nft={currentItemData} onClose={closeModal} />

      {!loading && nfts && nfts.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {nfts.map((nft) => (
            <NFTCard key={nft.id} nft={nft} isOwner={true} openModal={openModal} />
          ))}
        </div>
      )}
